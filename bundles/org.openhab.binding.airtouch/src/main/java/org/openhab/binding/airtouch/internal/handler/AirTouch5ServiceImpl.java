...
public class AirTouch5ServiceImpl implements AirTouchService<MessageConstants.Address> {
    ...
    private final Logger logger = LoggerFactory.getLogger(AirTouch5ServiceImpl.class);
    ...
    private AirtouchConnectorThreadFactory threadFactory = new Airtouch5ConnectorThreadFactory();
    ...
    public void requestFullUpdate() throws IOException {
        ...
        this.responseReceived.put(nextRequestId, Boolean.FALSE);
        myairtouchConnector.sendRequest(ZoneNameHandler.generateRequest(nextRequestId, null));

        nextRequestId = counter.incrementAndGet();
        this.responseReceived.put(nextRequestId, Boolean.FALSE);
        myairtouchConnector.sendRequest(AirConditionerStatusHandler.generateRequest(nextRequestId));
    ...
    public void validateZonePowerState(int zoneNumber, ZonePower zonePowerState) throws IllegalArgumentException {
        if (ZonePower.TURBO_POWER.equals(zonePowerState)
